...
private const val TAG = "ZiplineApi[getServerDataCounts]"

suspend fun getServerDataCounts(
    context: Context,
): Result<GetServerDataCountsResponse> {
    try {
        ...
        val response = ZiplineApiClient.getZiplineApiService(serverUrl).getServerDataCounts(
            token = token,
        )
        ...
        if (body is GetServerDataCountsResponse) {
