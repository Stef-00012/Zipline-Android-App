...
suspend fun downloadExport(
    ...
    notificationTitle: String = "Downloading export",
    ...
    return try {
        val response = ZiplineApiClient.getZiplineApiService(serverUrl).downloadExport(token, exportId)

        if (!response.isSuccessful) {
            service.failTransfer(transferId, "HTTP ${response.code()}")

            return Result.failure(Exception("Download export failed: HTTP ${response.code()}"))
        }

        val body = response.body() ?: run {
            service.failTransfer(transferId, "Empty response")

            return Result.failure(Exception("Download export failed: empty response body"))
        }
