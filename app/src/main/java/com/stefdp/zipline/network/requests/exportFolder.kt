...
suspend fun exportFolder(
    ...
    notificationTitle: String = "Exporting folder",
    ...
    return try {
        val response = ZiplineApiClient.getZiplineApiService(serverUrl).exportFolder(token, folderId)

        if (!response.isSuccessful) {
            service.failTransfer(transferId, "HTTP ${response.code()}")

            return Result.failure(Exception("Folder export failed: HTTP ${response.code()}"))
        }

        val body = response.body() ?: run {
            service.failTransfer(transferId, "Empty response")

            return Result.failure(Exception("Folder export failed: empty response body"))
        }
