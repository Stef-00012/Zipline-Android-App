...
suspend fun verifyFilePassword(
    ...
    try {
        ...
        val requestBody = VerifyFilePasswordBody(
            password = password
        )

        val response = ZiplineApiClient.getZiplineApiService(serverUrl).verifyFilePassword(
            token = token,
            fileId = fileId,
            data = requestBody
        )
        ...
        if (body is VerifyFilePasswordResponse) {
