...
private const val TAG = "ZiplineApi[bulkUpdateFiles]"
...
): Result<BulkUpdateFilesResponse> {
    try {
        ...
        val requestBody = BulkUpdateFilesBody(
            files = files,
            favorite = favorite,
            folderId = folderId
        )

        val response = ZiplineApiClient.getZiplineApiService(serverUrl).bulkUpdateFiles(
            token = token,
            data = requestBody
        )
        ...
        if (body is BulkUpdateFilesResponse) {
