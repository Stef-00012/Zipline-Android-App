...
private const val TAG = "ZiplineApi[addFileToFolder]"
...
suspend fun addFileToFolder(
    ...
    try {
        ...
        val requestBody = AddFileToFolderBody(
            fileId = fileId
        )

        val response = ZiplineApiClient.getZiplineApiService(serverUrl).addFileToFolder(
            token = token,
            folderId = folderId,
            data = requestBody
