...
private const val TAG = "ZiplineApi[deleteExport]"

suspend fun deleteExport(
    context: Context,
    exportId: String,
): Result<DeleteExportResponse> {
    try {
        ...
        val response = ZiplineApiClient.getZiplineApiService(serverUrl).deleteExport(
            token = token,
            exportId = exportId
        )
        ...
        if (body is DeleteExportResponse) {
