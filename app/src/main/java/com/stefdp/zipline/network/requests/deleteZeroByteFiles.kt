...
private const val TAG = "ZiplineApi[deleteZeroByteFiles]"

// ZeroByteFilesResponse will only contain "status"
suspend fun deleteZeroByteFiles(
    ...
    try {
        ...
        val response = ZiplineApiClient.getZiplineApiService(serverUrl).deleteZeroByteFiles(
            token = token,
        )
