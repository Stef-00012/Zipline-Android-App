...
private const val TAG = "ZiplineApi[runRequerySizeJob]"
...
suspend fun runRequerySizeJob(
    ...
    try {
        ...
        val requestBody = RunRequerySizeJobBody(
            forceDelete,
            forceUpdate
        )

        val response = ZiplineApiClient.getZiplineApiService(serverUrl).runRequerySizeJob(
            token = token,
            data = requestBody
        )
