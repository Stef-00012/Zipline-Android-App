...
private const val TAG = "ZiplineApi[scanForZeroByteFiles]"

// ZeroByteFilesResponse will only contain "files"
suspend fun scanForZeroByteFiles(
    context: Context
): Result<ZeroByteFilesResponse> {
    try {
        ...
        val response = ZiplineApiClient.getZiplineApiService(serverUrl).scanForZeroByteFiles(
            token = token,
        )
        ...
        if (body is ZeroByteFilesResponse) {
