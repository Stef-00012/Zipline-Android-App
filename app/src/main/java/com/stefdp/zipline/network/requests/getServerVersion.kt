...
private const val TAG = "ZiplineApi[getServerVersion]"
...
suspend fun getServerVersion(
    ...
    try {
        ...
        val response = ZiplineApiClient.getZiplineApiService(serverUrl).getServerVersion(
            token = token
        )
        ...
        if (body is GetServerVersionResponse) {
